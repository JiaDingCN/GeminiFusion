...
class ImVoteNet(Base3DDetector):
    ...
    def with_img_bbox(self):
        ...
        return (hasattr(self, "img_roi_head") and self.img_roi_head.with_bbox) or (
            hasattr(self, "img_bbox_head") and self.img_bbox_head is not None
        )
        ...
        if points is None:
            ...
            if self.with_img_rpn:
                proposal_cfg = self.train_cfg.get(
                    "img_rpn_proposal", self.test_cfg.img_rpn
                )
            ...
            for single_results in results_2d:
                cat_preds = torch.cat(
                    (
                        ...
                        single_results.scores[:, None],
                        single_results.labels[:, None],
                    ),
                    dim=-1,
                )
                cat_preds = cat_preds[torch.argsort(cat_preds[:, 4], descending=True)]
                # drop half bboxes during training for better generalization
                if self.training:
                    rand_drop = torch.randperm(len(cat_preds))[
                        : (len(cat_preds) + 1) // 2
                    ]
            ...
            losses_pts = self.pts_bbox_head_pts.loss(
                points, feat_dict_pts, batch_data_samples
            )
            losses_img = self.pts_bbox_head_img.loss(
                points, feat_dict_img, batch_data_samples
            )
                ...
                if "loss" in loss_term:
                    ...
                    for i in range(len(losses_towers)):
                        combined_losses[loss_term] += (
                            losses_towers[i][loss_term] * self.loss_weights[i]
                        )
                else:
        ...
        **kwargs,
        ...
        if points is None:
            ...
            results_3d = self.pts_bbox_head_joint.predict(
                batch_inputs_dict["points"], feat_dict, batch_data_samples, rescale=True
            )

            return self.add_pred_to_datasample(batch_data_samples, results_3d)

    def predict_img_only(
        self,
        imgs: Tensor,
        batch_data_samples: List[Det3DDataSample],
        rescale: bool = True,
