...
class HardSimpleVFE(nn.Module):
    ...
    def forward(
        self, features: Tensor, num_points: Tensor, coors: Tensor, *args, **kwargs
    ) -> Tensor:
        """Forward function.
...
class DynamicSimpleVFE(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, features: Tensor, coors: Tensor, *args, **kwargs) -> Tensor:
        """Forward function.
...
class DynamicVFE(nn.Module):
    ...
    def map_voxel_center_to_point(
        self, pts_coors: Tensor, voxel_mean: Tensor, voxel_coors: Tensor
    ) -> Tensor:
...
@MODELS.register_module()
class DynamicVFE_modified(nn.Module):
    """Dynamic Voxel feature encoder used in DV-SECOND.
    ...
    def __init__(
        ...
        super(DynamicVFE_modified, self).__init__()
        assert mode in ["avg", "max"]
        assert len(feat_channels) > 0
        if with_cluster_center:
            in_channels += 3
        ...
        if fusion_layer is not None:
            self.fusion_layer = []
            mid_fusion_layer = dict()
            for key, value in fusion_layer.items():
                mid_fusion_layer[key] = value
            mid_fusion_layer["mid_channels"] = mid_fusion_layer["pts_channels"]
            mid_fusion_layer["out_channels"] = mid_fusion_layer["pts_channels"]
            for i in range(len(self.vfe_layers)):
                if i != len(self.vfe_layers) - 1:
                    self.fusion_layer.append(MODELS.build(mid_fusion_layer))
                else:
                    self.fusion_layer.append(MODELS.build(fusion_layer))
    ...
    def forward(
        ...
        **kwargs
        ...
        for i, vfe in enumerate(self.vfe_layers):
            ...
            # Every Layer
            if self.fusion_layer[i] is not None and img_feats is not None:
                point_feats = self.fusion_layer[i](
                    img_feats, points, point_feats, img_metas
                )
...
class SegVFE(nn.Module):
    ...
    def __init__(
        ...
        self.vfe_layers = nn.ModuleList(vfe_layers)
        self.vfe_scatter = DynamicScatter(
            self.voxel_size, self.point_cloud_range, (mode != "max")
        )
    ...
    def forward(
        self, features: Tensor, coors: Tensor, *args, **kwargs
    ) -> Tuple[Tensor]:
        """Forward functions.
