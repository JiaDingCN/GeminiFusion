...
class Gemini_PointFusion(BaseModule):
    ...
    def __init__(
        ...
        super(Gemini_PointFusion, self).__init__(init_cfg=init_cfg)
        ...
        self.cross_attn_0_to_1 = nn.MultiheadAttention(
            mid_channels, self.cross_heads, dropout=0.0, batch_first=False
        )
        ...
        self.relation_judger = nn.Sequential(
            Mlp_2(mid_channels * 2, mid_channels, mid_channels),
            torch.nn.Softmax(dim=-1),
        )

        self.k_noise = nn.Embedding(2, mid_channels)
        self.v_noise = nn.Embedding(2, mid_channels)
    ...
    def forward(
        ...
        judger_input = torch.cat([x0.unsqueeze(0), x1.unsqueeze(0)], dim=-1)
        ...
        v = torch.cat([noise_v, x1.unsqueeze(0)], dim=0)

        new_x0 = x0 + self.cross_attn_0_to_1(q, k, v)[0].squeeze(0)

        ## 2. 1_to_0 cross attn and skip connect
        q = x1.unsqueeze(0)

        judger_input = torch.cat([x1.unsqueeze(0), x0.unsqueeze(0)], dim=-1)
        ...
        k = torch.cat([noise_k, torch.mul(q, relation_score)], dim=0)
        v = torch.cat([noise_v, x0.unsqueeze(0)], dim=0)

        new_x1 = x1 + self.cross_attn_1_to_0(q, k, v)[0].squeeze(0)

        img_pre_fuse = new_x0
        pts_pre_fuse = new_x1

        alpha_soft = F.softmax(self.alpha)
        fuse_out = alpha_soft[0] * img_pre_fuse + alpha_soft[1] * pts_pre_fuse
