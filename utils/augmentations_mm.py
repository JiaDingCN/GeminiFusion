...
class RandomAdjustSharpness:
    ...
    def __call__(self, sample: list) -> list:
        if random.random() < self.p:
            sample["rgb"] = TF.adjust_sharpness(sample["rgb"], self.sharpness)
        return sample
...
class RandomAutoContrast:
    ...
    def __call__(self, sample: list) -> list:
        if random.random() < self.p:
            sample["rgb"] = TF.autocontrast(sample["rgb"])
        return sample
...
class RandomResizedCrop:
    ...
    def __call__(self, sample: list) -> list:
        ...
        x2 = x1 + tW
        for k, v in sample.items():
            # print("before_1:", k, sample[k].shape)
            if len(v.shape) == 3:
                sample[k] = v[:, y1:y2, x1:x2]
            else:
                sample[k] = v[y1:y2, x1:x2]
            # print("after_1:", k, sample[k].shape)
        ...
        if sample["rgb"].shape[1:] != self.size:
            padding = [
                0,
                0,
                tW - sample["rgb"].shape[2],
                tH - sample["rgb"].shape[1],
            ]
            for k, v in sample.items():
                if k == "mask":
