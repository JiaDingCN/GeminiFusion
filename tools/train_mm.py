...
def main(cfg, gpu, save_dir, drop_path_rate):
    start = time.time()
    best_mIoU = 0.0
    best_epoch = 0
    num_workers = 0 if "DEBUG" in cfg.keys() and cfg["DEBUG"] else 8
    ...
    valset = eval(dataset_cfg["NAME"])(
        dataset_cfg["ROOT"], "val", valtransform, dataset_cfg["MODALS"]
    )
    ...
    if train_cfg["DDP"]:
        ...
        sampler_val = None
        model = DDP(
            model, device_ids=[gpu], output_device=0, find_unused_parameters=True
        )
    else:
    ...
    if (train_cfg["DDP"] and torch.distributed.get_rank() == 0) or (
        not train_cfg["DDP"]
    ):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for iter, (sample, lbl) in pbar:
            ...
            with autocast(enabled=train_cfg["AMP"]):
                if model_cfg["NAME"] == "GeminiFusion":
                    loss = 0.0
        ...
        if (
            (epoch + 1) % train_cfg["EVAL_INTERVAL"] == 0
            and (epoch + 1) > train_cfg["EVAL_START"]
        ) or (epoch + 1) == epochs:
            if (train_cfg["DDP"] and torch.distributed.get_rank() == 0) or (
                not train_cfg["DDP"]
            ):
                acc, macc, _, _, ious, miou = evaluate(cfg, model, valloader, device)
                writer.add_scalar("val/mIoU", miou, epoch)

                if miou > best_mIoU:
                    prev_best_ckp = (
                        save_dir
                        / f"{model_cfg['NAME']}_{model_cfg['BACKBONE']}_{dataset_cfg['NAME']}_epoch{best_epoch}_{best_mIoU}_checkpoint.pth"
                    )
                    prev_best = (
                        save_dir
                        / f"{model_cfg['NAME']}_{model_cfg['BACKBONE']}_{dataset_cfg['NAME']}_epoch{best_epoch}_{best_mIoU}.pth"
                    )
                ...
                logger.info(
                    f"Current epoch:{epoch} mIoU: {miou} Best mIoU: {best_mIoU}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cfg",
        type=str,
        default="configs/deliver_rgbdel.yaml",
    ...
    main(cfg, gpu, save_dir, args.drop_path_rate)
    cleanup_ddp()
